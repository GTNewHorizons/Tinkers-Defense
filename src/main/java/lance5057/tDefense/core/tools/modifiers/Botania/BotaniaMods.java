...
public class BotaniaMods extends ModifiersBase {
    ...
    public void RegisterRecipes() {
        CraftingManager.getInstance()
                .getRecipeList()
                .add(new ShapedOreRecipe(new ItemStack(modItems, 1, 0), new Object[] {
                    "fsf",
                    "oio",
                    "fgf",
                    'f',
                    ...
                    'i',
                    new ItemStack(ModItems.keepIvy, 1, 0),
                    's',
    ...
    public void RegisterModifiers() {
        ...
        for (final ToolCore tool : TConstructRegistry.getToolMapping()) {
            TConstructClientRegistry.addEffectRenderMapping(
                    tool, TinkersDefense.config.CorpseIvyModID, "tinker", "corpseivy", true);
            TConstructClientRegistry.addEffectRenderMapping(
                    tool, TinkersDefense.config.TerraCoreModID, "tinker", "terracore", true);
            TConstructClientRegistry.addEffectRenderMapping(
                    tool, TinkersDefense.config.ManaRepairModID, "tinker", "manacore", true);
        }
    }
    ...
    public void UpdateTerraCore(ToolCore tool, ItemStack stack, World world, Entity entity, NBTTagCompound tags) {
        if (entity instanceof EntityPlayer) {
            ...
            if (player.getCurrentEquippedItem() == stack
                ...
                final int color =
                        TConstructRegistry.getMaterial(tags.getInteger("Head")).primaryColor();
                final EntityManaBurst burst =
                        ((ItemTerraSword) ModItems.terraSword).getBurst(player, new ItemStack(ModItems.terraSword));
