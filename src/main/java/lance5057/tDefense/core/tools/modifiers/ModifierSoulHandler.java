...
public class ModifierSoulHandler {
    ...
    public void Update(ItemStack tool) {
        ...
        if (time <= 0) {
            if (!tags.getBoolean("Broken")) {
                ...
                final List<EntityLiving> entities = world.getEntitiesWithinAABB(
                        EntityLiving.class,
                ...
                // react entity
                    ...
                    if (entities.size() > 0) {
                        final EntityLiving ent = entities.get(world.rand.nextInt(entities.size()));

                        if (ent instanceof EntityCreature || ent instanceof EntityFlying
                                || ent instanceof EntitySlime) {
                            final boolean doGeneral = world.rand.nextBoolean();
                            final boolean doInsult = world.rand.nextBoolean();
                            if (!doInsult) {
                                if (!doGeneral) {
                                    output += "§7(" + ent.getClass().getName()
                                            .substring(ent.getClass().getName().lastIndexOf("Entity") + 6).toLowerCase()
                                            + " Nearby)§f ";
                                    output += talk(
                                            output,
                                            world,
                                            ...
                                            ent.getClass().getName()
                                                    .substring(ent.getClass().getName().lastIndexOf("Entity") + 6)
                                                    .toLowerCase() + "near");
                                } else {
                                ...
                                if (!doGeneral) {
                                    output += "§7(insult" + ent.getClass().getName()
                                            .substring(ent.getClass().getName().lastIndexOf("Entity") + 6).toLowerCase()
                                            + ")§f ";
                                    output += talk(
                                            output,
    ...
    void formatOutput(String output, EntityPlayer player, ItemStack tool) {
        ...
        if (output.contains("~material~")) {
            output = output.replace(
                    ...
                    TConstructRegistry.toolMaterials
                            .get(tool.getTagCompound().getCompoundTag("InfiTool").getInteger("Head")).localizedName());
        }
        ...
        if (!output.isEmpty()) {
            ...
            if (TinkersDefense.config.debug) {
                player.addChatMessage(new ChatComponentText("Error: Soul missing line"));
            }
        }
    }
    ...
    public void ModifySoulEvent(ModifyEvent event) {
        if (event.modifier.stacks != null && !event.modifier.stacks.isEmpty()
                && event.modifier.stacks.get(0) != null
                && ItemStack.areItemStacksEqual(
                        (ItemStack) event.modifier.stacks.get(0),
                        new ItemStack(Modifiers.TMod.modItems, 1, 3))) {
            newlymade = true;
    ...
    // event.entityLiving.worldObj.rand.nextInt(lines));
    // }
