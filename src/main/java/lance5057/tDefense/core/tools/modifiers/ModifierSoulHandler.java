...
public class ModifierSoulHandler {
    ...
    public void ModifySoulEvent(ModifyEvent event) {
        if (event.modifier.stacks != null
                && !event.modifier.stacks.isEmpty()
                && event.modifier.stacks.get(0) != null
                && ItemStack.areItemStacksEqual(
                        (ItemStack) event.modifier.stacks.get(0), new ItemStack(Modifiers.TMod.modItems, 1, 3))) {
            newlymade = true;
