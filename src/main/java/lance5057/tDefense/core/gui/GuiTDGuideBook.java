...
public class GuiTDGuideBook extends GuiScreen {
    ...
    public void initGui() {
        ...
        page = new Page(
                ...
                (DynamicToolPart) TinkerTools.frypanHead,
                null);
        pages.add(page);
        ...
        buttonList.add(new BookButton(0, ((width - 256) / 2) + 3, 240, false, 80, 0, 18, 12));
        buttonList.add(new BookButton(1, ((width + 256) / 2) - 3 - 18, 240, true, 80, 0, 18, 12));
        buttonList.add(new BookButton(2, ((width - 14) / 2), 238, true, 120, 0, 14, 13));
    ...
    static class Chapter {
        ...
        public BookButton getButton(int id, int w, int h) {
            return new BookButton(id, ((w + 256) / 2) - 8, 64 + 15 + (id - 3) * 22, true, 0, (id - 3) * 22, 35, 22);
        }
    }
    ...
    static class Page {
        ...
        public void drawText(FontRenderer fontRendererObj, int w, int h) {
            final int titleSize = fontRendererObj.getStringWidth(
                    StatCollector.translateToLocal("book.chapter" + chapter + ".page" + page + ".titleL"));
            fontRendererObj.drawString(
                    StatCollector.translateToLocal("book.chapter" + chapter + ".page" + page + ".titleL"),
                    ((w - 256) / 2) + 64 - titleSize,
                    64 + 14,
                    0);
            ...
            drawCenteredSplit(
                    ((w - 256) / 2) + 68,
                    84,
                    StatCollector.translateToLocal("book.chapter" + chapter + ".page" + page + ".ContentL"),
                    fontRendererObj);
        }

        public void drawTextRight(FontRenderer fontRendererObj, int w, int h) {
            final int titleSize = fontRendererObj.getStringWidth(
                    StatCollector.translateToLocal("book.chapter" + chapter + ".page" + page + ".titleR"));
            fontRendererObj.drawString(
                    StatCollector.translateToLocal("book.chapter" + chapter + ".page" + page + ".titleR"),
                    ((w + 256) / 2) - 72 - titleSize,
                    64 + 14,
                    0);
            ...
            drawCenteredSplit(
                    ((w + 256) / 2) - 72,
                    84,
                    StatCollector.translateToLocal("book.chapter" + chapter + ".page" + page + ".ContentR"),
                    fontRendererObj);
        }
        ...
        private void drawForgePage(GuiScreen gui, int numOfCrafts, int w, int h, FontRenderer fontRendererObj,
            ...
            itemRender.renderItemAndEffectIntoGUI(
                    ...
                    new ItemStack(TCRecipe.get(0), 1, TCItemCycle.get(0)),
                    w / 2 + 45,
                    h / 2 - 37);
            itemRender.renderItemAndEffectIntoGUI(
                    fontRendererObj,
                    gui.mc.getTextureManager(),
                    new ItemStack(TCRecipe.get(1), 1, TCItemCycle.get(1)),
                    w / 2 + 27,
                    h / 2 - 45);
            itemRender.renderItemAndEffectIntoGUI(
                    fontRendererObj,
                    gui.mc.getTextureManager(),
                    new ItemStack(TCRecipe.get(2), 1, TCItemCycle.get(2)),
                    w / 2 + 28,
                    h / 2 - 27);
            ...
            if (TCRecipe.size() <= 3) {
                ...
                itemRender.renderItemAndEffectIntoGUI(
                        fontRendererObj,
                        gui.mc.getTextureManager(),
                        new ItemStack(TCRecipe.get(3), 1, TCItemCycle.get(3)),
                        w / 2 + 11,
                        h / 2 - 37);
                itemRender.renderItemAndEffectIntoGUI(
                        fontRendererObj,
    ...
    static IRecipe lookUpRecipe(ItemStack stack) {
        ...
        while (recipe.hasNext()) {
            ...
            if (next != null) {
                if (next.getItem() == stack.getItem()) return recipe.next();
            }
        }
        return null;
    }
            ...
            for (int i = 0; i < 9; i++) {
                if (SRecipe.recipeItems.get(i) != null) {
                    ...
                    itemRender.renderItemAndEffectIntoGUI(
                            font,
                            tex,
                            (ItemStack) SRecipe.recipeItems.get(i),
                            w / 2 + x + ((i % 3) * 17),
                            h / 2 + y + (i / 3 * 17));
                }
            }
