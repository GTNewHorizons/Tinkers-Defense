...
public class TDEventHandler {
    ...
    public void craftTDPart(PartBuilderEvent.NormalPart event) {
        ...
        if (event.pattern.getItem() == TinkersDefense.woodPattern && event.pattern.getItemDamage() == 3) {
            ...
            final ClothMaterial mat = (ClothMaterial) TConstructRegistry
                    .getCustomMaterial(event.material, ClothMaterial.class);
    ...
    public void TD_ShieldBlock(ShieldBlockEvent event) {
        if (event.shield.getItem() instanceof RoundShield) {
            ...
            entity.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 5, 10));
        }
    }
    ...
    // event.entityLiving.posY, event.entityLiving.posZ, new ItemStack(TinkersDefense.item_RedMintcane,1,0)));
    // if(event.entityLiving.getHeldItem().getItem() == TinkersDefense.item_GreenMintcane)
    // event.drops.add(new EntityItem(event.entityLiving.worldObj, event.entityLiving.posX,
    // event.entityLiving.posY, event.entityLiving.posZ, new ItemStack(TinkersDefense.item_GreenMintcane,1,0)));
    //
    // }
    // }
