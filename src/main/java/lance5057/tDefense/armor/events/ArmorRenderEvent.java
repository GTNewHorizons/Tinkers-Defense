...
public class ArmorRenderEvent {
    ...
    private void TrimArmor(ModelBiped armorModel, RenderPlayerEvent.SetArmorModel event) {
        if (armorModel != null) {
            ...
            final float yaw = event.entityPlayer.prevRotationYawHead
                    + (event.entityPlayer.rotationYawHead - event.entityPlayer.prevRotationYawHead)
                            * event.partialRenderTick;
            final float yawOffset = event.entityPlayer.prevRenderYawOffset
                    + (event.entityPlayer.renderYawOffset - event.entityPlayer.prevRenderYawOffset)
                            * event.partialRenderTick;
            final float limbs = event.entityPlayer.prevLimbSwingAmount
                    + (event.entityPlayer.limbSwingAmount - event.entityPlayer.prevLimbSwingAmount)
                            * event.partialRenderTick;
            final float limbSwing = event.entityPlayer.limbSwing
                    - event.entityPlayer.limbSwingAmount * (1.0F - event.partialRenderTick);

            armorModel.setRotationAngles(
                    limbSwing,
                    limbs,
                    event.entityPlayer.ticksExisted,
                    yaw - yawOffset,
