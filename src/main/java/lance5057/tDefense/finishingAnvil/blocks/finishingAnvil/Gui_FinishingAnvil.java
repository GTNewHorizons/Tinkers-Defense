...
public class Gui_FinishingAnvil extends GuiContainer {
    ...
    private final RenderItem bigRender = new RenderItem_FinishingAnvil(this, 3.5f, 3.5f, 1.0f);

    private ItemStack rightCopy1, rightCopy2, rightCopy3;
    NBTTagCompound rightCopyTags1, rightCopyTags2, rightCopyTags3;
    private final RenderItem normalRender = new RenderItem_FinishingAnvil(this, 1.0f, 1.0f, 1.0f);
    ...
    public void initGui() {
        ...
        buttonList.add(new GuiButtonExt(13, guiLeft + 120, guiTop + 11, 18, 18, "3D"));
    ...
    protected void actionPerformed(GuiButton button) {
        if (bigCopy != null && bigCopyTags != null) {
            switch (button.id) {
                ...
                case 3:
                    TinkersDefense.INSTANCE.sendToServer(
                            new Message_FinishingAnvil(inventory.xCoord, inventory.yCoord, inventory.zCoord, bigCopy));
                    break;
        ...
        switch (id) {
            case 9:
                if (bigCopyTags.hasKey("Render" + renders[leftSelect])) {
                    bigCopyTags.setInteger(
                            "Render" + renders[leftSelect],
                            bigCopy.getTagCompound().getCompoundTag("InfiTool").getInteger(renders[leftSelect])
                                    + (rightButtonPos * TinkersDefense.config.MaterialIndex));
                    if (rightButtonPos > 0) {
                        bigCopyTags.setInteger(
                                renders[leftSelect] + "Color",
                                TConstructRegistry.getMaterial(bigCopyTags.getInteger(renders[leftSelect]))
                                        .primaryColor());
                    } else {
            ...
            case 10:
                if (bigCopyTags.hasKey("Render" + renders[leftSelect])) {
                    bigCopyTags.setInteger(
                            "Render" + renders[leftSelect],
                            bigCopy.getTagCompound().getCompoundTag("InfiTool").getInteger(renders[leftSelect])
                                    + ((rightButtonPos + 1) * TinkersDefense.config.MaterialIndex));
                    bigCopyTags.setInteger(
                            renders[leftSelect] + "Color",
                            TConstructRegistry.getMaterial(bigCopyTags.getInteger(renders[leftSelect])).primaryColor());
                }
                break;
            ...
            case 11:
                if (bigCopyTags.hasKey("Render" + renders[leftSelect])) {
                    bigCopyTags.setInteger(
                            "Render" + renders[leftSelect],
                            bigCopy.getTagCompound().getCompoundTag("InfiTool").getInteger(renders[leftSelect])
                                    + ((rightButtonPos + 2) * TinkersDefense.config.MaterialIndex));
                    bigCopyTags.setInteger(
                            renders[leftSelect] + "Color",
                            TConstructRegistry.getMaterial(bigCopyTags.getInteger(renders[leftSelect])).primaryColor());
                }
            ...
            case 12:
                bigCopyTags.setInteger(
                        "RenderHead",
                        bigCopy.getTagCompound().getCompoundTag("InfiTool").getInteger(renders[0]));
                bigCopyTags.setInteger(
                        "RenderAccessory",
                        bigCopy.getTagCompound().getCompoundTag("InfiTool").getInteger(renders[1]));
                bigCopyTags.setInteger(
                        "RenderHandle",
                        bigCopy.getTagCompound().getCompoundTag("InfiTool").getInteger(renders[2]));
                bigCopyTags.setInteger(
                        "RenderExtra",
                        bigCopy.getTagCompound().getCompoundTag("InfiTool").getInteger(renders[3]));
    ...
    public void doArmorEdit(int id) {
        ...
        switch (id) {
            case 9:
                aTags.setBoolean(
                        boxes.get(rightButtonPos + 9).boxName,
                        !aTags.getBoolean(boxes.get(rightButtonPos + 9).boxName));
                break;

            case 10:
                aTags.setBoolean(
                        boxes.get(rightButtonPos + 1 + 9).boxName,
                        !aTags.getBoolean(boxes.get(rightButtonPos + 1 + 9).boxName));
                break;

            case 11:
                aTags.setBoolean(
                        boxes.get(rightButtonPos + 2 + 9).boxName,
    ...
    public void drawScreen(int x, int y, float par3) {
        ...
        if (inventory.getStackInSlot(0) != null && inventory.getStackInSlot(0).getItem() instanceof ToolCore) {
            if (bigCopy == null) {
                ...
                setRenders();
            } else if (bigCopy != null && inventory.getStackInSlot(0).getItem() != bigCopy.getItem()
                    || inventory.getStackInSlot(0).getItemDamage() != bigCopy.getItemDamage()) {
                        bigCopy = null;
                    }
        ...
        if (bigCopy != null) {
            ...
            if (!(bigCopy.getItem() instanceof ArmorCore)) {
                if (renderFlat) {
                    ...
                    normalRender
                            .renderItemAndEffectIntoGUI(fontRendererObj, mc.getTextureManager(), rightCopy1, 153, 13);
                    normalRender
                            .renderItemAndEffectIntoGUI(fontRendererObj, mc.getTextureManager(), rightCopy2, 153, 33);
                    normalRender
                            .renderItemAndEffectIntoGUI(fontRendererObj, mc.getTextureManager(), rightCopy3, 153, 53);
                }
            } else {
                ...
                // drawTexturedModalRect(153, 13, xRIcon_one + (rightButtonPos * 16), yRIcon_one + (rightButtonPosY *
                // 16), 16, 16);

                drawTexturedModalRect(
                        153,
                        13,
                        64 + ((aTags.getBoolean(boxes.get(rightButtonPos + 9).boxName) ? 0 : 1) * 16),
                        176,
                        16,
                        16);
                drawTexturedModalRect(
                        153,
                        33,
                        64 + ((aTags.getBoolean(boxes.get(rightButtonPos + 1 + 9).boxName) ? 0 : 1) * 16),
                        176,
                        16,
                        16);
                drawTexturedModalRect(
                        153,
                        53,
                        64 + ((aTags.getBoolean(boxes.get(rightButtonPos + 2 + 9).boxName) ? 0 : 1) * 16),
                        176,
                        16,
                        16);
            }
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (!renderFlat) {
            if (bigCopy != null && bigCopy.getItem() instanceof ArmorCore) {
                ...
                final float limbs = player.player.prevLimbSwingAmount
                        + (player.player.limbSwingAmount - player.player.prevLimbSwingAmount);
                ...
                GL11.glViewport(
                        (scaledresolution.getScaledWidth() - 213) / 2 * scaledresolution.getScaleFactor(),
                ...
                // renderer.setRotationAngles(limbSwing, limbs, player.player.ticksExisted, 0,
                // player.player.rotationPitch, 0.0625f, player.player);
                ...
                if (renderBiped) {
                    ...
                    // final ITextureObject tex =
                    // FMLClientHandler.instance().getClient().getTextureManager().getTexture(rc);
    ...
    protected void drawTooltip(int x, int y) {
        ...
        for (int i = 0; i < buttons.length; i++) {
            ...
            final boolean mousehover = x >= button.xPosition && y >= button.yPosition
            ...
            if (test == 2) {
                ...
                if (bigCopy.getItem() instanceof ArmorCore) {
                    ...
                    final String toolName = tool.getUnlocalizedName().toLowerCase().substring(
                            tool.getUnlocalizedName().lastIndexOf('.') + 1,
                            tool.getUnlocalizedName().length());
